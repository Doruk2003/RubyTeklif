...
module Products
  class IndexQueryTest < ActiveSupport::TestCase
    ...
    test "returns pagination metadata and builds paged query" do
      ...
      result = Products::IndexQuery.new(client: client).call(params: { page: "2", per_page: "5", category: "cat-1" })

      assert_equal 5, result[:items].size
      assert_equal true, result[:has_next]
      assert_equal true, result[:has_prev]
      assert_includes client.paths.first, "category_id"
      assert_includes client.paths.first, "category_id=eq.cat-1"
