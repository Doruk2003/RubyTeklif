...
class AdminUsersControllerExportTest < ActionDispatch::IntegrationTest
  class FakeAuth
  ...
  test "queues users export job for admin" do
    ...
    with_admin_auth do |user_id|
      with_stubbed_constructor(Admin::Users::IndexQuery, FakeIndexQuery.new) do
        original_perform_later = Admin::Users::ExportCsvJob.method(:perform_later)
        Admin::Users::ExportCsvJob.singleton_class.send(:define_method, :perform_later) do |token, actor_id, filters|
          fake_job.perform_later(token, actor_id, filters)
        end

        post export_admin_users_path, params: { role: Roles::MANAGER, active: "true" }
        assert_redirected_to admin_users_path(role: Roles::MANAGER, active: "true")
